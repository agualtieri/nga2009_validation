...
check_log <- function(data, log, variable = "question.name", old_log_var = "old.value", new_log_var = "new.value", uuid_data = "uuid", uuid_log = "uuid") {
  ...
  extract_value <- function(data, 
   ...
   final2 <- final %>% mutate(., check = ifelse(identical(unname(new_log_var), unname(variable)), "Log applied correctly", "Please check log")) 
   
   #%>%
                                      #     filter(check == "Please check log")
     
 #  ifelse( (stringi::stri_compare(new_log_var, value_extracted) == 0), "Log applied correctly", "Please check log")) %>%
 # filter(check == "Please check log")
     
  #   filter(final, !(new_log_var == value_extracted))
